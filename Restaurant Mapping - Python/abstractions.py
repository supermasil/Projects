...
def make_user(name, reviews):
    ...
    #Returns [user_name, {R_name: review}]
...
def user_reviews(user):
    ...
    return user[1] #Returns {R_name: review}
...
def user_reviewed_restaurants(user, restaurants):
    ...
    """
    names = list(user_reviews(user)) # a list of [{R_name: review}]
    return [r for r in restaurants if restaurant_name(r) in names]
       # Loops R_name in restaurants and returns a list of the names showing up in [{R_name: review}]
def user_rating(user, restaurant_name):
    """Return the rating given for restaurant_name by user."""
    reviewed_by_user = user_reviews(user) #Returns the dictionary {R_name: [R_name, rating]}
    user_review = reviewed_by_user[restaurant_name] #Returns [R_name, rating]
    return review_rating(user_review) #Returns rating
