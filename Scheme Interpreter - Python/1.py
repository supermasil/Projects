...
class Pair:
    ...
    def map(self, fn):
        ...
        mapped = fn(self.first) ## Evaluate the FIRST first
        ## Pair(Pair(Pair....)) >> Every pair will be evaluated the same way
...
def scheme_read(src):
    ...
    if src.current() is None:
        raise EOFError
    val = src.remove_front() # Get and remove the first token
    if val == 'nil':
        # BEGIN PROBLEM 1
        "*** YOUR CODE HERE ***"
        return nil
        ...
        ## '(1 2) >>> Pair('quote', Pair(Pair(1, Pair(2, nil)), nil))
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            current_elem = scheme_read(src) ## Read through the expression after the dot
            if src.current() == ')':
                src.remove_front() ## Remove the closing parenthesis
                return current_elem
            else:
            ...
            "*** YOUR CODE HERE ***" 
            if src.current() == '(':
                return Pair(scheme_read(src), read_tail(src)) ## Pair(Pair(...))
            elif src.current() == "'":
                return Pair(scheme_read(src), read_tail(src))
                ## Read_tail(src) will take care of the rest of the parentheses
                ## (car '(1 2)) >>> Pair(car, Pair(Pair('quote', Pair(Pair(1 Pair(2, nil)), nil)), nil))
            else:
                return Pair(src.remove_front(), read_tail(src)) ## Pair(a, Pair(b, nil))
            # END PROBLEM 1
    except EOFError:
        raise SyntaxError('unexpected end of file')
