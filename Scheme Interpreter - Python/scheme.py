...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    elif self_evaluating(expr): ## Number, bool, nil...
        return expr
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS: ## and, or, if, define...
        return SPECIAL_FORMS[first](rest, env) ## Look up in dict and call the function
    else:
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        operator = scheme_eval(first, env) ## Evaluate the operator. This will raise an ERROR if not
        check_procedure(operator) ## check if it is an operator
        return operator.eval_call(rest, env) ## Call eval_call to evaluate the operands
...
def eval_all(expressions, env): ## Use to evaluate primitive expressions
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        try:
            ...
            if self.parent is not None: ## If it is NONE, it will exit this except and RAISE
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Number of formal parameters doesn\'t match number of values')
        else:
            while formals is not nil: ## Loop through the Scheme List to define them
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        args = operands.map(lambda x: scheme_eval(x, env)) ## Evaluate operands. Use lambda because scheme_eval receives more than 1 args
                ## OPERANDS is a PAIR object so need to use DOT to call MAP
        return scheme_apply(self, args, env) ## Apply the operator on the operands (Now is a scheme list of primitive operands)
                            ## SELF is the operator itself
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Wrong number of parameters passed in')
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        ## ....     (+ x (* y 2) (* z 3)))
        ## ....   (inner x 10))
        ## (outer 1 2) >> UserDefinedProcedure's apply func will be called with args are Pair(Pair(1, Pair(2, nil)), nil)
        return self.env.make_child_frame(self.formals, args)
        ## We need to copy and bind (x y) which are in self.env to (1 2) in a child of self.env
        ## (outer 1 2) >>> SCHEME_EVAL >>> Detect it is a UserDedine func >>> APPLY func in UserDefineProcedure
        ## >> EVAL_ALL the body >>> define the inner >> (inner x 10) is called >>> Same process applies
        # END PROBLEM 12
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        "*** YOUR CODE HERE ***"
        #print(operands)
        #args = operands.map(lambda x: complete_eval(x)) ## Evaluate operands. Use lambda because scheme_eval receives more than 1 args
                ## OPERANDS is a PAIR object so need to use DOT to call MAP
       #print(args)
        wrapping = scheme_apply(self, operands, env) ## Apply the operator on the operands (Now is a scheme list of primitive operands)
                            ## SELF is the operator itself
        unwrapping = complete_eval(wrapping)
        return scheme_eval(unwrapping, env)
...
def do_define_form(expressions, env):
    ...
    check_form(expressions, 2) ## Check if it is a well-formed list with min len of 2
    target = expressions.first ## Pair(A, Pair(B, nil)) >> TARGET is A
    if scheme_symbolp(target): ## Check if it is a valid symbol
        check_form(expressions, 2, 2) ## min = max = 2 >> Only binding, not for defining functions
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        # BEGIN PROBLEM 6
...
def do_quote_form(expressions, env):
    ...
    check_form(expressions, 1, 1) ## Only one pair of parentheses is approved
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
...
def do_if_form(expressions, env):
    """Evaluate an if form."""
    check_form(expressions, 2, 3) ## Min len of 2 and max len of 3
    if scheme_truep(scheme_eval(expressions.first, env)): ## If the condition evaluates to True
        return scheme_eval(expressions.second.first, env, True) ## Everything but False is True
    elif len(expressions) == 3: ## If it has another argument in the body and the cond is False
        return scheme_eval(expressions.second.second.first, env, True) ## Return the other arg -> Else case

def do_and_form(expressions, env):
    """Evaluate a (short-circuited) and form."""
    # BEGIN PROBLEM 13
    "*** YOUR CODE HERE ***"
    if expressions is nil: ## (and)
        return True

    if expressions.second is nil:
        eval = scheme_eval(expressions.first, env, True)
    else:
        eval = scheme_eval(expressions.first, env) ## Can only eval once or the result won't be the same for Suite 1 case 3

    if eval is False: ##(and False) ## Need to use is, can't use ==
                                            ## 0 == False >> True, 0 is False >> to False
                                            ## False is False >> True
        return False
    elif expressions.second == nil:
        return eval                 ## (and 1 2 3) >> 3
    else:
...
def do_or_form(expressions, env):
    ...
    if expressions is nil: ## (or) (or False)
        return False
    ...
    if eval is not False: ##(or False 3) >> 3 or (or 1) >> 1
        return eval
    else:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        clause = expressions.first ## The first test
        check_form(clause, 1) ## Min len of 1 (else)
        if clause.first == 'else':
            test = True
            if expressions.second != nil:
                raise SchemeError('else must be last')
        else:
            test = scheme_eval(clause.first, env) ## Eval what's inside the test
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        raise SchemeError('Improper form')
    # END Problem 21
...
class MuProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        ## Create a child env to the env MU was called
   # END PROBLEM 16
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            operator = scheme_eval(first, env) ## Evaluate the operator. This will raise an ERROR if not
            check_procedure(operator) ## check if it is an operator
            result =  operator.eval_call(rest, env) ## Call eval_call to evaluate the operands
