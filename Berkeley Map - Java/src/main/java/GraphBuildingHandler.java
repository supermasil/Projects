...
public class GraphBuildingHandler extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
        ...
        if (qName.equals("node")) {
            ...
            currentNode = new GraphDB.Node(attributes.getValue("id"), attributes.getValue("lat"),
                    attributes.getValue("lon"));
            g.addNode(currentNode); // Add the node
            g.addNodetoNode(currentNode); // Create adjacent list

        } else if (qName.equals("way")) {
            /* We encountered a new <way...> tag. */
            activeState = "way";
            currentWay = new GraphDB.Way(attributes.getValue("id"));

        } else if (activeState.equals("way") && qName.equals("nd")) {
            /* While looking at a way, we found a <nd...> tag. */
            //lastNd = Long.parseLong(attributes.getValue("ref"));
            nd.addLast(Long.parseLong(attributes.getValue("ref")));
            ...
            if (k.equals("maxspeed") || k.equals("name") || k.equals("highway")) {
                /* Figure out whether this way and its connections are valid. */
                /* Hint: Setting a "flag" is good enough! */
                if (ALLOWED_HIGHWAY_TYPES.contains(v)) {
                    isHighWay = true;
                }
                currentWay.extraInfo.put(k, v);
            }
        } else if (qName.equals("tag")) { // FOR NODES
            if (activeState.equals("node") && attributes.getValue("k").equals("name")) {
                /* While looking at a node, we found a <tag...> with k="name". */
                /* Create a location. */
            /* Hint: Since we found this <tag...> INSIDE a node, we should probably remember which
            node this tag belongs to. Remember XML is parsed top-to-bottom, so probably it's the
            last node that you looked at (check the first if-case). */
                currentNode.extraInfo.put(attributes.getValue("k"), attributes.getValue("v"));
                //System.out.println(attributes.getValue("v"));
                g.addTrie(attributes.getValue("v")); // For the cleanedLocation map
                g.addCleanedName(attributes.getValue("v"), currentNode); // For the Trie
            }
        }
    }
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
        if (qName.equals("way")) {
            ...
            if (isHighWay) {
                g.addWay(currentWay);
                long previous = nd.getFirst();
                for (long l : nd) {
                    if (l != previous) {
                        g.retrieveNodeToNode(l).put(previous, currentWay.id);
                        g.retrieveNodeToNode(previous).put(l, currentWay.id);
                        // Connect the previous nd to the current one
                    }
                    previous = l;
                }
            }
